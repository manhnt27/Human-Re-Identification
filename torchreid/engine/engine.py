...
class Engine(object):
    ...
    def run(
        ...
        if test_only:
            self.test(
                ...
                ranks=ranks,
                rerank=rerank,
                num_ids_from_id=num_ids_from_id,
                threshold=threshold,
    ...
    def test(
        ...
        for name in targets:
            ...
            rank1, mAP, idf1_list = self._evaluate(
                dataset_name=name,
            ...
            if self.writer is not None:
                for i in range(len(idf1_list)):
                    self.writer.add_scalar(f'Test/{name}/IDF1', idf1_list[i], i)
    ...
    def _evaluate(
        ...
        idf1_list = metrics.evaluate_idf1(
            distmat,
            q_pids,
            g_pids,
            q_camids,
        ...
        mean_idf1 = sum(idf1_list) / len(idf1_list)
        print('IDF1: {:.2%}'.format(mean_idf1))
